...
def test_joint_positioning(ann_wrapper):
    ...
    writer = {}
    for name in part_enc:
        reader[name] = Joint_Reader.PyJointReader()
        # init joint reader
        reader[name].init(ann_wrapper, name, name, sigma, n_pop, neuron_res)

        # add joint writer instances
        writer[name] = Joint_Writer.PyJointWriter()
        writer[name].init(ann_wrapper, name, name, n_pop, neuron_res, 25.0)
    ...
    jnts = {}
    jnts['right_arm'] = writer["right_arm"].get_joint_count()
    jnts['left_arm'] = writer["left_arm"].get_joint_count()
    ...
    for name in part_enc:
        ...
        write_joint_each(ann_wrapper, "double_single", "abs", True, name, positions, writer[name].write_double_one, reader[name].read_double_one, check_position_double_mult, path, jnts)
        write_joint_each(ann_wrapper, "double_single", "rel", True, name, positions, writer[name].write_double_one, reader[name].read_double_one, check_position_double_mult, path, jnts, rel_pos_double, positions)

        print('__ Type of positioning: Double multiple')
        write_joint_mult(ann_wrapper, "double_mult", "abs", True, name, positions, writer[name].write_double_multiple, reader[name].read_double_one, check_position_double_mult, path, jnt_sel)
        write_joint_mult(ann_wrapper, "double_mult", "rel", True, name, positions, writer[name].write_double_multiple, reader[name].read_double_one, check_position_double_mult, path, jnt_sel, rel_pos_double, positions)

        print('__ Type of positioning: Double all')
        write_joint_all(ann_wrapper, "double_all", "abs", True, name, positions, writer[name].write_double_all, reader[name].read_double_all, check_position_double_all, path)
        write_joint_all(ann_wrapper, "double_all", "rel", True, name, positions, writer[name].write_double_all, reader[name].read_double_all, check_position_double_all, path, rel_pos_double, positions)
        ...
        write_joint_each(ann_wrapper, "pop_single", "abs", True, name, part_enc, writer[name].write_pop_one, reader[name].read_pop_one, check_position_pop_mult, path, jnts)
        # write_joint_each(ann_wrapper, "pop_single", "rel", True, name, part_enc, writer[name].write_pop_one, reader[name].read_pop_one, check_position_pop_mult, path, jnts, rel_pos_pop, positions)

        print('__ Type of positioning: Population multiple')
        # write_joint_mult(ann_wrapper, "pop_mult", "abs", True, name, part_enc, writer[name].write_pop_multiple, reader[name].read_pop_one, check_position_pop_mult, path, jnt_sel)
        # write_joint_mult(ann_wrapper, "pop_mult", "rel", True, name, part_enc, writer[name].write_pop_multiple, reader[name].read_pop_one, check_position_pop_mult, path, jnt_sel, rel_pos_pop, positions)

        print('__ Type of positioning: Population all')
        write_joint_all(ann_wrapper, "pop_all", "abs", True, name, part_enc, writer[name].write_pop_all, reader[name].read_pop_all, check_position_pop_all, path)
        # write_joint_all(ann_wrapper, "pop_all", "rel", True, name, part_enc, writer[name].write_pop_all, reader[name].read_pop_all, check_position_pop_all, path, rel_pos_pop, positions)
    ...
    for part in part_enc:
        reader[part].close(ann_wrapper)
        writer[part].close(ann_wrapper)
...
def test_tactile_reading(ann_wrapper):
    ...
    # init skin reader
    print("Init:", sreader.init(ann_wrapper, "SReader", "r", True))
    print("Init:", sreader1.init(ann_wrapper, "SReader1", "R", False))
    ...
    for i in range(len(loc_sph) - 1):
        ...
        if d_pos.sum() != 0:
            for j in range(data_count):
                ...
                sreader1.read_tactile()

                # return the read data from the interface
                data_arm_norm.append(sreader.get_tactile_arm())
                data_farm_norm.append(sreader.get_tactile_forearm())
                data_hand_norm.append(sreader.get_tactile_hand())
    ...
    # return the read data from the interface
    data_arm_raw = sreader1.get_tactile_arm()
    data_farm_raw = sreader1.get_tactile_forearm()
    data_hand_raw = sreader1.get_tactile_hand()
    ...
    # close skin reader
    sreader.close(ann_wrapper)
    sreader1.close(ann_wrapper)
