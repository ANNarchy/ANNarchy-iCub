...
class ClientInstance {
 ...
 public:
    ...
    std::vector<int> retrieve_rgbimage() {
        iCubInterfaceMessages::ImageRequest request;
        iCubInterfaceMessages::RgbImageResponse response;

        grpc::ClientContext context;

        // auto state = stub_->ReadTest(&context, request, &response);
        auto state = stub_->RetrieveImage(&context, request, &response);

        if (state.ok()) {
            return std::vector<int>(response.imagel().begin(), response.imagel().end());
        } else {
            std::cerr << "ClientInstance::retrieve_image() failed: " << state.error_message() << std::endl;
            return std::vector<int>(320 * 240, 0.0);    // TODO: make dependent from Population geometry
        }
    }
