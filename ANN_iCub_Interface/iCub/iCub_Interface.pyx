            ...
            if not robot == None:
                ...
                for vread in robot.iter('VisReader'):
                    ...
                    if(no_error_vread):
                        ...
                        if grpc:
                            if not vreader.init_grpc(self, vread.attrib['name'], args['eye'], args['fov_width'], args['fov_height'], args['img_width'], args['img_height'], args['fast_filter'], ini_path=args['ini_path'], ip_address=args['ip_address'] , port=args['port']):
                                print("Init Visual Reader '" + vread.attrib['name'] + "' failed!")
                        else:
                            if not vreader.init(self, vread.attrib['name'], args['eye'], args['fov_width'], args['fov_height'], args['img_width'], args['img_height'], args['fast_filter'], ini_path=args['ini_path']):
                                print("Init Visual Reader '" + vread.attrib['name'] + "' failed!")
                    else:
